...
class Parser {
    constructor (data) {
        ...
        _.forEach(requiredKeys, (key) => {
            if (!_.contains(dataKeys, key)) throw new Error('Invalid data format');
        });

        Object.keys(data.browsers).forEach((key) => {
            data.browsers[key].short = stripHtmlTags(data.browsers[key].short);
        });
...
module.exports.stripHtmlTags = stripHtmlTags;
